...
class TestInstitutionReorder:
    """Drag-and-drop institution reordering — independent per page."""

    def test_get_order_unauthenticated_returns_401(self, client):
        res = client.get("/api/accounts/institutions/order?page=dashboard")
        assert res.status_code == 401

    def test_reorder_unauthenticated_returns_401(self, client):
        res = client.patch(
            "/api/accounts/institutions/reorder",
            json={"page": "dashboard", "names": []},
        )
        assert res.status_code == 401

    def test_reorder_persists_and_returns(self, client, auth_headers):
        """Reorder stores names for one page; GET returns them in saved order."""
        names = ["Chase", "Robinhood", "Voya"]
        res = client.patch(
            "/api/accounts/institutions/reorder",
            headers=auth_headers,
            json={"page": "dashboard", "names": names},
        )
        assert res.status_code == 200
        ...
        order = client.get(
            "/api/accounts/institutions/order?page=dashboard", headers=auth_headers
        ).json()
        assert order["names"] == names

    def test_reorder_replaces_previous_for_same_page(self, client, auth_headers):
        """A second reorder call for the same page wipes the prior list."""
        client.patch("/api/accounts/institutions/reorder", headers=auth_headers,
                     json={"page": "dashboard", "names": ["A", "B", "C"]})
        client.patch("/api/accounts/institutions/reorder", headers=auth_headers,
                     json={"page": "dashboard", "names": ["X", "Y"]})
        order = client.get(
            "/api/accounts/institutions/order?page=dashboard", headers=auth_headers
        ).json()
    ...
    def test_reorder_strips_empty_names(self, client, auth_headers):
        ...
        client.patch("/api/accounts/institutions/reorder", headers=auth_headers,
                     json={"page": "dashboard", "names": ["Real", "", "  ", "Also Real"]})
        order = client.get(
            "/api/accounts/institutions/order?page=dashboard", headers=auth_headers
        ).json()
        assert order["names"] == ["Real", "Also Real"]

    def test_pages_are_independent(self, client, auth_headers):
        """Saving an order on one page must not change the other page's order."""
        client.patch("/api/accounts/institutions/reorder", headers=auth_headers,
                     json={"page": "dashboard", "names": ["Chase", "Voya"]})
        client.patch("/api/accounts/institutions/reorder", headers=auth_headers,
                     json={"page": "accounts", "names": ["Voya", "Chase", "Robinhood"]})

        dash = client.get(
            "/api/accounts/institutions/order?page=dashboard", headers=auth_headers
        ).json()
        accts = client.get(
            "/api/accounts/institutions/order?page=accounts", headers=auth_headers
        ).json()
        assert dash["names"] == ["Chase", "Voya"]
        assert accts["names"] == ["Voya", "Chase", "Robinhood"]

        # Updating one page leaves the other untouched
        client.patch("/api/accounts/institutions/reorder", headers=auth_headers,
                     json={"page": "dashboard", "names": ["Voya", "Chase"]})
        dash2 = client.get(
            "/api/accounts/institutions/order?page=dashboard", headers=auth_headers
        ).json()
        accts2 = client.get(
        ...
        assert dash2["names"] == ["Voya", "Chase"]
        assert accts2["names"] == ["Voya", "Chase", "Robinhood"]

    def test_invalid_page_rejected_on_get(self, client, auth_headers):
        res = client.get(
            "/api/accounts/institutions/order?page=bogus", headers=auth_headers
        )
        assert res.status_code == 400

    def test_invalid_page_rejected_on_reorder(self, client, auth_headers):
        res = client.patch(
            "/api/accounts/institutions/reorder",
            headers=auth_headers,
            json={"page": "bogus", "names": ["X"]},
        )
        assert res.status_code == 400

    def test_missing_page_param_on_get_rejected(self, client, auth_headers):
